...
function Initialize-StudioAPI {
    ...
    # Workflows now live under content/workflows/, so DotbotHome is two levels up.
    # Tolerate a non-nested layout (used by some tests) by checking the parent name.
    $parent = Split-Path $WorkflowsDir -Parent
    if ((Split-Path $parent -Leaf) -eq 'content') {
        $script:DotbotHome = Split-Path $parent -Parent
    } else {
        $script:DotbotHome = $parent
    }
