/**
 * Client-side JSON parsing, serialization, and validation for workflow manifests.
 */
import type { WorkflowManifest } from '../model/workflow';

/**
 * Parse a workflow.json string into a WorkflowManifest.
 */
export function parseWorkflowJson(content: string): WorkflowManifest {
  const doc = JSON.parse(content) as WorkflowManifest;
  if (!doc) {
    throw new Error('Empty or invalid JSON document');
  }
  return doc;
}

/**
 * Serialize a WorkflowManifest back to JSON string.
 */
export function serializeWorkflowJson(manifest: WorkflowManifest): string {
