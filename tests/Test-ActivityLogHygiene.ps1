...
.DESCRIPTION
    Covers three of the four sub-bugs in #367:
      ...
      3. Invoke-DotbotProcess.ps1 resets DOTBOT_CORRELATION_ID at startup so
...
Assert-True -Name "Invoke-DotbotProcess.ps1 sets DOTBOT_CORRELATION_ID unconditionally" `
    -Condition ($launchProcessSource -match '\$env:DOTBOT_CORRELATION_ID\s*=\s*"corr-') `
    -Message "Expected an unconditional reset of DOTBOT_CORRELATION_ID in Invoke-DotbotProcess.ps1"

Assert-True -Name "Invoke-DotbotProcess.ps1 reset is not gated by a `if (-not env:DOTBOT_CORRELATION_ID)` check" `
