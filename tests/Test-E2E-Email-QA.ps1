...
<#
.SYNOPSIS
    Layer 4: End-to-end test for the Email Q&A round-trip with attachments.
.DESCRIPTION
    For each fixture (markdown, PDF, image), creates a fresh dotbot project
    configured against a real DotbotServer on the Email channel, then exercises
    two reply paths:

      Path A: inject a simulated reply via POST /api/test/responses
              (same channel-agnostic endpoint the Teams test uses) and
    ...
    Path B is the email-specific flow that real recipients follow when they
    click the magic link in the outbound email.

    Required env: DOTBOT_SERVER_URL, DOTBOT_API_KEY, DOTBOT_EMAIL_RECIPIENT.
    Optional: DOTBOT_REQUIRE_REAL_DELIVERY=true gates the Tier 3 assertion
    against instance.sentTo[*].status == "sent".

    The server must be started with DOTBOT_TEST_MODE=true and have
    DeliveryChannels:Email:Enabled=true in its appsettings.
#>
...
Write-Host "  Layer 4: E2E Email Q&A with Attachments" -ForegroundColor Blue
Write-Host "═══════════════════════════════════════════════════════════" -ForegroundColor Blue
Write-Host ""
...
$emailRecipient  = $env:DOTBOT_EMAIL_RECIPIENT
...
if ($missing.Count -gt 0) {
    Write-TestResult -Name "Layer 4 Email prerequisites" -Status Skip `
        -Message "Missing env var(s): $($missing -join ', ')"
    Write-TestSummary -LayerName "Layer 4: E2E Email Q&A" | Out-Null
    exit 0
}

if ($emailRecipient -notmatch '@') {
    Write-TestResult -Name "Layer 4 Email prerequisites" -Status Fail `
        -Message "DOTBOT_EMAIL_RECIPIENT must be an email address (contain '@'): got '$emailRecipient'"
    Write-TestSummary -LayerName "Layer 4: E2E Email Q&A" | Out-Null
    exit 1
}
...
Write-Host "    Channel  : email" -ForegroundColor DarkGray
Write-Host "    Recipient: $emailRecipient" -ForegroundColor DarkGray
Write-Host ""
...
foreach ($f in $fixtures) {
    Assert-PathExists -Name "Email: fixture '$($f.File)' present" -Path (Join-Path $fixturesDir $f.File)
}

function Invoke-EmailRoundTrip {
    param(
    ...
    $label = $Fixture.Label
    $testProject = New-TestProject -Prefix "dotbot-email-$($Fixture.Key)"
    ...
    try {
        ...
        if (-not (Test-Path $botDir)) {
            Write-TestResult -Name "Email[$label]: .bot initialized" -Status Fail -Message ".bot missing after init-project.ps1"
            return
        }
        ...
        $control = [ordered]@{
            ...
            mothership  = [ordered]@{
                ...
                project_name          = "dotbot-email-test-$label"
                project_description   = "Layer 4 Email round-trip fixture: $label"
        ...
        $taskContent = [pscustomobject]@{
            id   = $taskId
            name = "Email E2E fixture: $label"
        }
        $pendingQuestion = [pscustomobject]@{
            id             = $questionLocalId
            question       = "Email E2E: please attach the $label fixture and choose 'accept'."
        ...
        if (-not $sendResult.success) {
            Write-TestResult -Name "Email[$label]: Send-TaskNotification succeeded" -Status Fail -Message $sendResult.reason
            return
        }
        Write-TestResult -Name "Email[$label]: Send-TaskNotification succeeded" -Status Pass
        ...
        try {
            ...
            Write-TestResult -Name "Email[$label]: Path A - injected simulated response" -Status Pass
        } catch {
            Write-TestResult -Name "Email[$label]: Path A - injected simulated response" -Status Fail -Message $_.Exception.Message
            return
        }
        ...
        if (-not $response) {
            Write-TestResult -Name "Email[$label]: Path A - response polled within 30s" -Status Fail -Message "Timed out polling /responses"
            return
        }
        Write-TestResult -Name "Email[$label]: Path A - response polled within 30s" -Status Pass
        ...
        if (-not $resolved) {
            Write-TestResult -Name "Email[$label]: Path A - Resolve-NotificationAnswer returned a result" -Status Fail -Message "null"
            return
        }
        Write-TestResult -Name "Email[$label]: Path A - Resolve-NotificationAnswer returned a result" -Status Pass
        Assert-Equal -Name "Email[$label]: Path A - resolved attachments count == 1" -Expected 1 -Actual @($resolved.attachments).Count

        $localFile = Join-Path $attachDir $Fixture.File
        Assert-PathExists -Name "Email[$label]: Path A - attachment saved at $($Fixture.File)" -Path $localFile
        if (Test-Path $localFile) {
            Assert-Equal -Name "Email[$label]: Path A - saved byte size matches fixture" -Expected $fixtureSize -Actual (Get-Item $localFile).Length
            Assert-Equal -Name "Email[$label]: Path A - SHA-256 matches fixture" `
        ...
        try {
            ...
            if (-not $mint.token) {
                Write-TestResult -Name "Email[$label]: Path B - minted magic-link token" -Status Fail -Message "No token returned"
                return
            }
            Write-TestResult -Name "Email[$label]: Path B - minted magic-link token" -Status Pass
        } catch {
            Write-TestResult -Name "Email[$label]: Path B - minted magic-link token" -Status Fail -Message $_.Exception.Message
            return
        }
        ...
        try {
            ...
            $multipart.Add([System.Net.Http.StringContent]::new("accept"), "selectedKey")
            $multipart.Add([System.Net.Http.StringContent]::new("Path B reply from $label fixture"), "freeText")
            ...
            if (-not $httpResponse.IsSuccessStatusCode) {
                ...
                Write-TestResult -Name "Email[$label]: Path B - POST /respond returned 2xx" -Status Fail `
                    -Message "HTTP $([int]$httpResponse.StatusCode): $($errBody.Substring(0, [Math]::Min(200, $errBody.Length)))"
                return
            }
            Write-TestResult -Name "Email[$label]: Path B - POST /respond returned 2xx" -Status Pass
        } catch {
            Write-TestResult -Name "Email[$label]: Path B - POST /respond" -Status Fail -Message $_.Exception.Message
            return
        } finally {
            $httpClient.Dispose()
            $httpHandler.Dispose()
        ...
        if ($pathBResolved -and @($pathBResolved.attachments).Count -eq 1) {
            ...
            if (Test-Path $pathBFile) {
                Assert-Equal -Name "Email[$label]: Path B - SHA-256 matches fixture" `
                    -Expected $sourceHash -Actual (Get-FileHash -Path $pathBFile -Algorithm SHA256).Hash
            } else {
                Write-TestResult -Name "Email[$label]: Path B - SHA-256 matches fixture" -Status Fail -Message "Downloaded file missing at $pathBFile"
            }
        } else {
            Write-TestResult -Name "Email[$label]: Path B - SHA-256 matches fixture" -Status Fail -Message "Could not resolve Path B attachment"
        }
        ...
        if ($env:DOTBOT_REQUIRE_REAL_DELIVERY -eq "true") {
            try {
                ...
                Assert-Equal -Name "Email[$label]: delivered to >= 1 recipient (Tier 3)" `
                    -Expected $true -Actual ($delivered.Count -ge 1)
            } catch {
                Write-TestResult -Name "Email[$label]: real delivery assertion (Tier 3)" -Status Fail -Message $_.Exception.Message
            }
        }
...
foreach ($f in $fixtures) {
    ...
    Write-Host "  ────────────────────────────────────────────" -ForegroundColor DarkGray
    Invoke-EmailRoundTrip -Fixture $f -ServerUrl $serverUrl -ApiKey $apiKey `
        -Recipient $emailRecipient -DotbotDir $dotbotDir -FixturesDir $fixturesDir
    Write-Host ""
}

$allPassed = Write-TestSummary -LayerName "Layer 4: E2E Email Q&A"
