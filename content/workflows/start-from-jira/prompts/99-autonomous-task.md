...
1. **Read the research methodology:**
   ```
   Read({ file_path: ".bot/content/workflows/start-from-jira/research/{analysis.research_prompt}" })
   ```
...
2. **Verify output quality** against the research-output standard:
   ```
   Read({ file_path: ".bot/content/workflows/start-from-jira/standards/global/research-output.md" })
   ```
