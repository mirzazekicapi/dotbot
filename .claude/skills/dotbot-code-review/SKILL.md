...
- **Agent 3 — Bug hunter.** Diff-only. Flag only what is verifiable inside the diff. No nitpicks. Apply the relevant subsection based on file extensions in the diff:
  ...
  **General (all languages):**
  ...
  - **Write-without-read:** when code writes data to a file path (config, prompt, state), verify that a consumer exists that reads from that exact path. Check the existing codebase for the canonical location of that data — if another path is already established (e.g., `.bot/.control/launchers/workflow-launch-prompt.txt` vs `workspace/product/workflow-launch-prompt.txt`), the new write may be orphaned.
